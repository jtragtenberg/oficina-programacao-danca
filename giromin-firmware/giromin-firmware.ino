...
const char* ssid     = "COLOCAR WIFI AQUI";
const char* password = "SENHA DO WIFI AQUI";

const int ID        = 1; //ID do Giromin que está trabalhando
const int BOTAO_PIN = 5;

IPAddress destIP(192, 168, 0, 101); //Colocar IP de seu computador na mesma rede Wi-Fi do Giromin
const int destPort  = 9999; //Configurar o software no computador para ler esta porta OSC
...
void setup() {
  ...
  Serial.println("Rede:  " + String(ssid));
  Serial.println("Senha: " + String(password));
  Serial.println("Conectando");
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) { delay(500); Serial.print("."); }
  Serial.println("\nIP do Giromin:    " + WiFi.localIP().toString());
  Serial.println("Enviando OSC para: " + destIP.toString() + ":" + String(destPort));
